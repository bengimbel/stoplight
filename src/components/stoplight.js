...
const StopLight = ({ configuration }) => {
  ...
  const handleTimer = useCallback(() => {
    const obj = configuration.sequence?.find((item) =>
      item?.colors?.includes(activeLight)
    );
  ...
  const handleListChange = useCallback(() => {
    const list = Object.entries(configuration.lights)
      .sort(([, vala], [, valb]) => vala.position - valb.position)
      .reduce((acc, [key, val]) => [...acc, { ...val, name: key }], []);
    setLightList(list);
  ...
  return (
    <div style={{ width: "5rem", height: "auto", background: "#000000" }}>
      ...
      >
        {isArray(lightList) &&
          Object.values(lightList).map((item) => (
            <Light
              key={item.name}
              isActive={item.name === activeLight}
              color={item.color}
            />
          ))}
      </div>
